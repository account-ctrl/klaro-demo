...
const FeatureMap = dynamic(
  ...
  { 
    ssr: false,
    loading: () => <div className="h-full w-full flex items-center justify-center bg-zinc-900 text-zinc-500">Loading Map Component...</div>
  }
);

function EmergencyCommandCenterPage() {
    const { incidents, loading: loadingIncidents } = useIncidents();
    const { responders } = useLiveLocations();
    ...
    return (
        <div className="relative w-screen h-screen overflow-hidden bg-zinc-950 text-white font-sans">
            ...
            {/* Right Panel */}
            <div className="absolute right-6 top-6 bottom-6 z-20 flex flex-col gap-4 w-80 pointer-events-none">
