...
import { useFirestore, useAuth, initiateEmailSignIn } from '@/firebase'; 
...
function OnboardingPageComponent() {
    ...
    useEffect(() => {
        if (!inviteToken) {
            toast({
                title: "Invalid Link",
                description: "The onboarding link is missing an invitation token. Please use the link provided in your email.",
                variant: "destructive",
                duration: 10000
            });
        }
    ...
    const handleCommissionStart = async () => {
        if (!inviteToken) {
            toast({ title: "Missing Token", description: "Cannot proceed without a valid invite token.", variant: "destructive" });
            return;
        }
        if (!isCertified) {
    ...
    if (!inviteToken) {
        return (
            <div className="flex min-h-screen items-center justify-center bg-zinc-950 p-4 text-zinc-100">
                <Card className="bg-zinc-900/50 border-zinc-800 shadow-2xl max-w-md text-center">
                    <CardHeader>
                        <div className="mx-auto bg-red-500/10 p-3 rounded-full w-fit">
                            <AlertTriangle className="h-8 w-8 text-red-500" />
                        </div>
                        <CardTitle className="text-2xl text-red-400">Invalid Onboarding Link</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <p className="text-zinc-400">
                            The invitation token is missing from the URL. This link is invalid or has expired. 
                            Please use the original, unmodified link from your invitation email to proceed.
                        </p>
                    </CardContent>
                    <CardFooter>
            ...
            <CardContent className="p-8">
                  ...
                  <form onSubmit={profileForm.handleSubmit(handleProfileSubmit)} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                      <div className="space-y-4">
                          <div className="flex items-center justify-between border-b border-zinc-800 pb-2 mb-4">
                              <h2 className="text-xl font-semibold flex items-center gap-2"><MapPin className="text-primary h-5 w-5" /> Node Location</h2>
                          ...
                          <div className="grid gap-6">
                              ...
                              <FormItem>
                                  ...
                                  <FormControl><div className="relative"><Input placeholder="Search Province..." {...field} className="bg-zinc-950/50 border-zinc-700 focus-visible:ring-primary pl-4" />{field.value.length > 3 && <CheckCircle2 className="absolute right-3 top-3 h-4 w-4 text-green-500" />}</div></FormControl>
                                  <FormMessage />
                              </FormItem>
                              )} />
                              ...
                              <FormItem className={field.value ? "" : "hidden"}> 
                                  ...
                                  <FormControl><div className="relative"><Input readOnly placeholder="Region Auto-Detected..." {...field} className="bg-zinc-900 border-zinc-800 text-zinc-500 focus-visible:ring-0 pl-4 cursor-not-allowed" /><CheckCircle2 className="absolute right-3 top-3 h-4 w-4 text-green-500" /></div></FormControl>
                                  <FormMessage />
                              </FormItem>
                              )} />
                              ...
                              <FormItem>
                                  ...
                                  <FormControl><div className="relative"><Input placeholder="Search City/Municipality..." {...field} className="bg-zinc-950/50 border-zinc-700 focus-visible:ring-primary pl-4" />{field.value.length > 3 && <CheckCircle2 className="absolute right-3 top-3 h-4 w-4 text-green-500" />}</div></FormControl>
                                  <FormMessage />
                              </FormItem>
                              )} />
                              ...
                              <FormItem>
                                  ...
                                  <FormControl><div className="relative"><Input placeholder="e.g. San Isidro" {...field} className="bg-zinc-950/50 border-zinc-700 focus-visible:ring-primary pl-4" />{field.value.length > 3 && <CheckCircle2 className="absolute right-3 top-3 h-4 w-4 text-green-500" />}</div></FormControl>
                                  <FormMessage />
                              </FormItem>
                              )} />
                          </div>
                      </div>
                      <div className="pt-4"><Button type="submit" className="w-full h-12 text-base font-semibold shadow-primary/20 shadow-lg">Verify & Continue <ArrowRight className="ml-2 h-4 w-4" /></Button></div>
                  </form>
                </Form>
              )}
              ...
              {currentStep === 2 && (
                <Form {...officialsForm}>
                  <form onSubmit={officialsForm.handleSubmit(handleOfficialsSubmit)} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                      <div className="flex items-center justify-between border-b border-zinc-800 pb-2 mb-4"><h2 className="text-xl font-semibold flex items-center gap-2"><Users className="text-primary h-5 w-5" /> Authorized Custodians</h2></div>
                          ...
                          <Card key={field.id} className="bg-zinc-900 border-zinc-800">
                              <CardContent className="p-4 flex gap-4 items-start">
                                  <div className="grid gap-4 flex-1">
                                      <FormField name={`officials.${index}.name`} control={officialsForm.control} render={({ field }) => (<FormItem><FormLabel className="text-xs text-zinc-500 uppercase">Official Name</FormLabel><FormControl><Input placeholder="Full Name" {...field} className="bg-zinc-950 border-zinc-700" /></FormControl><FormMessage /></FormItem>)} />
                                      <FormField name={`officials.${index}.role`} control={officialsForm.control} render={({ field }) => (<FormItem><FormLabel className="text-xs text-zinc-500 uppercase">Role / Designation</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger className="bg-zinc-950 border-zinc-700"><SelectValue placeholder="Select Role" /></SelectTrigger></FormControl><SelectContent><SelectItem value="Captain">Punong Barangay (Captain)</SelectItem><SelectItem value="Secretary">Barangay Secretary</SelectItem><SelectItem value="Treasurer">Barangay Treasurer</SelectItem><SelectItem value="Councilor">Barangay Kagawad</SelectItem></SelectContent></Select><FormMessage /></FormItem>)} />
                                      {officialsForm.watch(`officials.${index}.role`) === 'Captain' && (
                                          <>
                                              <FormField name={`officials.${index}.email`} control={officialsForm.control} render={({ field }) => (<FormItem><FormLabel className="text-xs text-zinc-500 uppercase">Email (For Login)</FormLabel><FormControl><Input placeholder="official@barangay.gov.ph" {...field} className="bg-zinc-950 border-zinc-700" /></FormControl><FormMessage /></FormItem>)} />
                                               <FormField name={`officials.${index}.password`} control={officialsForm.control} render={({ field }) => (<FormItem><FormLabel className="text-xs text-zinc-500 uppercase">Password</FormLabel><FormControl><Input type="password" placeholder="******" {...field} className="bg-zinc-950 border-zinc-700" /></FormControl><FormMessage /></FormItem>)} />
                                          </>
                                      )}
                                  </div>
                                  <Button type="button" variant="ghost" size="icon" className="text-zinc-500 hover:text-red-400 mt-6" onClick={() => remove(index)} disabled={fields.length <= 1}><Trash2 className="h-4 w-4" /></Button>
                              </CardContent>
                          </Card>
                          ))}
                      </div>
                      <Button type="button" variant="outline" className="w-full border-dashed border-zinc-700 text-zinc-400 hover:text-white hover:bg-zinc-800" onClick={() => append({ name: '', role: 'Councilor' })}><PlusCircle className="mr-2 h-4 w-4" /> Add Another Official</Button>
              ...
              {currentStep === 3 && (
                  <div className="animate-in fade-in slide-in-from-right-4 duration-500 space-y-8">
                       <div className="flex items-center justify-between border-b border-zinc-800 pb-2 mb-4"><h2 className="text-xl font-semibold flex items-center gap-2"><Stamp className="text-primary h-5 w-5" /> Digital Seal Generation</h2></div>
                      <div className="grid md:grid-cols-2 gap-8 items-center">
                          <div className="space-y-6">
                               <div className="bg-zinc-950/50 p-6 rounded-lg border border-zinc-800 text-center border-dashed">
                                  ...
                                  <label htmlFor="logo-upload"><Button variant="secondary" size="sm" className="pointer-events-none" asChild><span>Select File</span></Button></label>
                               </div>
                               <div className="text-xs text-zinc-500"><p>This logo will be used to generate a standardized high-resolution digital seal for official documents.</p></div>
                          </div>
                          <div className="flex flex-col items-center justify-center space-y-4">
                               ...
                               <div className="relative w-48 h-48 rounded-full border-4 border-double border-yellow-600 bg-white flex items-center justify-center shadow-2xl overflow-hidden">
                                  {logoPreview ? <img src={logoPreview} alt="Seal" className="w-32 h-32 object-contain" /> : <div className="text-center text-zinc-300"><FileBadge className="h-16 w-16 mx-auto text-zinc-200" /></div>}
              ...
              {currentStep === 4 && (
                  <div className="animate-in fade-in slide-in-from-right-4 duration-500 space-y-6">
                      <div className="flex items-center justify-between border-b border-zinc-800 pb-2 mb-4">
                          <h2 className="text-xl font-semibold flex items-center gap-2"><ShieldCheck className="text-primary h-5 w-5" /> Pre-Commissioning Review</h2>
                      ...
                      <div className="bg-zinc-950 p-6 rounded-lg border border-zinc-800 space-y-4 font-mono text-sm">
                          <div className="flex justify-between"><span className="text-zinc-500">Node ID:</span><span className="text-zinc-200">{profileData ? `${profileData.province.substring(0,3).toUpperCase()}-${profileData.city.substring(0,3).toUpperCase()}-${profileData.barangayName.replace(/\s/g,'').toUpperCase()}` : 'PENDING'}</span></div>
                           <div className="flex justify-between"><span className="text-zinc-500">Jurisdiction:</span><span className="text-zinc-200">{profileData ? `${profileData.barangayName}, ${profileData.city}, ${profileData.province}` : '...'}</span></div>
                          <div className="flex justify-between"><span className="text-zinc-500">Primary Custodian:</span><span className="text-zinc-200">{officialsData?.officials.find(o => o.role === 'Captain')?.name || '...'}</span></div>
                           <div className="flex justify-between"><span className="text-zinc-500">Security Level:</span><span className="text-green-500">HIGH (Level 4)</span></div>
                      </div>
                      <div className="flex items-start space-x-3 bg-primary/5 p-4 rounded-lg border border-primary/10">
                          <Checkbox id="certify" checked={isCertified} onCheckedChange={(c) => setIsCertified(c as boolean)} className="mt-1"/>
                          <div className="grid gap-1.5 leading-none">
                              <label htmlFor="certify" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-zinc-200">I certify that I am an authorized representative of this government unit.</label>
                              <p className="text-xs text-zinc-500">I understand that impersonating a government official is a punishable offense under the Revised Penal Code of the Philippines.</p>
                          </div>
                      </div>
                      <div className="flex gap-4 pt-4">
                          <Button type="button" variant="ghost" onClick={() => setCurrentStep(3)} className="w-1/3">Back</Button>
                          <Button onClick={handleCommissionStart} disabled={!isCertified} className={cn("w-2/3 h-12 shadow-lg transition-all", isCertified ? "shadow-primary/20 bg-primary hover:bg-primary/90" : "opacity-50")}><TerminalSquare className="mr-2 h-4 w-4" /> Commission Node</Button>
                      </div>
                  </div>
              )}
              ...
              {currentStep === 5 && (
                  <div className="flex flex-col items-center justify-center py-10 animate-in fade-in zoom-in duration-500">
                      ...
                      <p className="mt-8 text-zinc-500 text-sm animate-pulse">{logs[logs.length-1]?.includes("COMPLETE") ? "Redirecting to secure dashboard..." : "Processing secure handshake..."}</p>
                  </div>
              )}
            </CardContent>
          </Card>
