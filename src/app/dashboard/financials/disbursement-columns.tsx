...
import { EditDisbursement, DeleteDisbursement } from "./disbursement-actions";
import { Badge } from "@/components/ui/badge";

type DisbursementTableActionsProps = {
  record: DV;
...
function DisbursementTableActions({ record, onEdit, onDelete, residents }: DisbursementTableActionsProps) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem onSelect={(e) => e.preventDefault()}>
          <EditDisbursement record={record} onEdit={onEdit} residents={residents} />
        </DropdownMenuItem>
        <DropdownMenuItem onSelect={(e) => e.preventDefault()} className="text-destructive focus:bg-destructive/10 focus:text-destructive">
          <DeleteDisbursement recordId={record.dvId} onDelete={onDelete} />
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
export const getColumns = (onEdit: (record: DV) => void, onDelete: (id: string) => void, residents: Resident[]): ColumnDef<DV>[] => [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <DisbursementTableActions record={record} onEdit={onEdit} onDelete={onDelete} residents={residents} />
    },
  },
];
